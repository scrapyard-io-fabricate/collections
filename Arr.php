...
class Arr
{
     ...
     *     ? true
     *     : ($value is Arrayable         ? true
     *         : ($value is Traversable
     *             ? true
     *             : ($value is Jsonable                 ? true
     ...
     * @return array
     */
    public static function dot(iterable $array, string $prepend = '', float|int $depth = INF): array
    {
        $results = [];
    ...
    /**
     * Get every given array except for a specified array of keys.
     *
     * @param array $array
     * @param float|int|array|string $keys
     * @return array
     */
    public static function except(array $array, float|int|array|string $keys): array
    {
    ...
    /**
     * Get every given array except for a specified array of values.
     *
     * @param array $array
     * @param  mixed  $values
     * @param bool $strict
     ...
     * @return bool
     */
    public static function exists(ArrayAccess|array $array, float|int|string $key): bool
    {
     ...
     */
    public static function first(array $array, ?callable $callback = null, ?Closure $default = null)
    {
        if (is_null($callback)) {
            if (empty($array)) {
     ...
     */
    public static function last(array $array, ?callable $callback = null, ?Closure $default = null)
    {
        if (is_null($callback)) {
     ...
     *
     * @param WeakMap<object, TValue>|JsonSerializable|iterable|Arrayable<TKey, TValue>|Enumerable<TKey, TValue>|Jsonable $items
     * @return ($items is WeakMap ? list<TValue> : array<TKey, TValue>)
     *
     * @throws InvalidArgumentException
     */
    public static function from(Arrayable|Enumerable|WeakMap|Jsonable|JsonSerializable|iterable $items): array
    {
        return match (true) {
     ...
     * @return bool
     */
    public static function hasAll(ArrayAccess|array $array, array|string $keys): bool
    {
        $keys = (array) $keys;
     ...
     * @return array
     */
    public static function keyBy(Arrayable|array $array, callable|array|string $keyBy): array
    {
     ...
     *
     * @param iterable $array
     * @param int|array|string|Closure|null $value
     * @param array|string|Closure|null $key
     * @return array
     */
    public static function pluck(iterable $array, int|array|string|Closure|null $value, array|string|Closure|null $key = null): array
    {
        $results = [];
     ...
     *
     * @param iterable<TKey, TValue> $array
     * @param callable|string|array<int, (callable(TValue, TValue): -1|0|1)|array{string, 'asc'|'desc'}>|null $callback
     * @return array<TKey, TValue>
     */
    ...
    public static function sortRecursive(array $array, int $options = SORT_REGULAR, bool $descending = false): array
    {
        ...
        if (! array_is_list($array)) {
            match ($descending) {
                false => ksort($array, $options),
                true => krsort($array, $options),
            };
        } else {
            match ($descending) {
                false => sort($array, $options),
                true => rsort($array, $options),
            };
        }
     ...
     *
     * @param string|array<string, bool>|array<int, string|int> $array
     ...
     * @template TValue
     *
     * @param  TValue|array<array-key, TValue>|null  $value
